...
- **THEN** a JSON array is returned where each item has id and label for: todo, in-progress, review, done
...
- **WHEN** POST /api/todos is called with a status of todo, in-progress, review, or done
...
- **WHEN** PUT /api/todos/:id is called with status todo, in-progress, review, or done
