...
- **THEN** it has id (UUID), title (string), status (todo|in-progress|review|done), createdAt (ISO date), updatedAt (ISO date), and dueDate (YYYY-MM-DD string or null)
...
- **THEN** status MUST be one of: todo, in-progress, review, done
